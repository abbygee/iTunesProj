...
function display(data) {
    ...
    var trackTime = millisToMinutesAndSeconds(data.results[song].trackTimeMillis);
    ...
    contain.append('<div>Released on: ' + releaseDate + '</div>');
    contain.append('<div>Track Time: ' + trackTime + '</div>');
    contain.append('<div> Genre: ' + data.results[song].primaryGenreName + '</div>');
    contain.append('<div><a href="' + data.results[song].collectionViewUrl + '" target="_blank">Apple Music Album Page</a></div>');
    ...
    contain.append('<div><a href="' + href +'">Go Back</a></div>');
}
